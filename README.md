...
The `output/` folder contains the results obtained after generating one synthetic family-structured survival data set

- `output/data/`: contains the synthetic family-structured data set generated by the script.
- `output/estimates/`: contains the parameter estimates obtained for each value of the penalty parameter.
- `output/variances/`: contains the robust asymptotic variance-covariance matrices obtained for each value of the penalty parameter.
- `output/all_results_by_gamma.rds`: contains all estimation results grouped by penalty parameter.

## Computational time

For each value of the penalty parameter gamma, parameter estimation and robust asymptotic variance calculation take approximately 20 minutes on average for one simulated data set.
